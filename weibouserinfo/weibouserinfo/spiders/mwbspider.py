...
class MwbspiderSpider(Spider):
    ...
    def parse_user(self, response):
        ...
        if result.get('data').get('userInfo'):
            ...
            # 微博
            yield Request(self.weibos_url.format(uid=uid, page=1), callback=self.parse_weibos,meta={'page': 1, 'uid': uid})
    ...
    def parse_weibos(self, response):
        ...
        if result.get('ok') and result.get('data').get('cards'):
            ...
            uid = response.meta.get('uid')
            page = response.meta.get('page') + 1
            yield Request(self.weibos_url.format(uid=uid, page=page), callback=self.parse_weibos,meta={'uid': uid, 'page': page})